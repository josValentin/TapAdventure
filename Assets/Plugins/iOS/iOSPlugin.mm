...
// get or load float values
+(float)iCloudGetFloatValue:(NSString *)key
{
    NSUbiquitousKeyValueStore *cloudStore = [NSUbiquitousKeyValueStore defaultStore];
    return [[cloudStore objectForKey:key] intValue];
}

+(BOOL)iCloudSaveFloatValue:(NSString *)key setValue:(float)value
{
    NSUbiquitousKeyValueStore *cloudStore = [NSUbiquitousKeyValueStore defaultStore];
    [cloudStore setLongLong:value forKey:key];
    return [cloudStore synchronize];
...
extern "C"
{
    ...
    float _iCloudGetFloatValue(const char *key)
    {
        return [iOSPlugin iCloudGetFloatValue:[NSString stringWithUTF8String:key]];
    }
    
    bool _iCloudSaveFloatValue(const char *key, float value)
    {
        return [iOSPlugin iCloudSaveFloatValue:[NSString stringWithUTF8String:key] setValue:value];
    }
